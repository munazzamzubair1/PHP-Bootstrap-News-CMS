    ...
    <div class="row">
      <div class="col-md-8">
        ...
        <div class="post-container">
          <?php
          $uid = isset($_GET['uid']) ? $_GET['uid'] : null; // Getting author id from url
          if ($db->select("user", "username", "", " user_id=$uid")) {
            $author_name = $db->getResult()[0]['username']; // Getting author name from database
          }
          ?>
          <h2 class="page-heading"><?php echo ucfirst($author_name); ?></h2>
          <?php
          ...
          if ($db->select("post", "title, category_name, category, description,  username, author, post_date, post_img, post_id, description", " user ON post.author = user.user_id JOIN category ON post.category = category.category_id", " user_id=$uid", "", " $offset, $limit")) { // If result contains atleast one record
          ...
          <ul class='pagination'>
            ...
            if ($db->pagination("post", "post_id", $limit, $currentFileName, "page", $page, " author=$uid", ["uid" => $uid])) {
